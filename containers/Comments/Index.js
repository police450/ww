...
class Comment extends React.Component {
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        ...
        }else if (nextProps.comment_item_id != prevState.id || nextProps.type != prevState.type) {
    ...
    componentDidUpdate(prevProps, prevState) {
        if(this.props.comment_item_id != prevProps.comment_item_id){
    ...
    componentDidMount() {
        ...
        if(!this.props.comments){
            ...
            this.props.socket.on('commentCreated', data => {
                ...
                if(this.props.pageData.loggedInUserDetails){
                    loggedinuserid = this.props.pageData.loggedInUserDetails.user_id
                }
            ...
            if(this.props.comment_item_id){
                $('html, body').animate({
                    'scrollTop' : $(`#comment-`+this.props.comment_item_id).position().top
                });
            }
        }
        ...
        this.props.socket.on('replyEdited', data => {
            ...
            if (id == this.state.id && type == this.state.type) {
                ...
                if (commentIndex > -1) {
                    ...
                    const replyIndex = this.getReplies(id, replies)
                    if (replyIndex > -1) {
        ...
        this.props.socket.on('replyCreated', data => {
            ...
            if (id == this.state.id && type == this.state.type) {
                ...
                if (comment && id) {
                    if(approve == 1 || owner_id == loggedinuserid){
                        const commentIndex = this.getComment(id)
                        if (commentIndex > -1) {
                            let comments = [...this.state.comments]
                            let replies = comments[commentIndex]['replies']
                            if (!replies) {
        ...
        if (comment_id) {
            ...
            if (commentIndex > -1) {
                ...
                if (replyIndex > -1) {
                    ...
                    this.setState({localUpdate:true, editing: true, editMessage: reply.message, editImage: (reply.image ? this.props.pageData.imageSuffix + reply.image : null), edit_comment_id: comment_id, edit_reply_comment_id: isReply })
                }
            }
        } else {
            //comment
            ...
            if (commentIndex > -1) {
                ...
                this.setState({localUpdate:true, editing: true, editMessage: comment.message, editImage: (comment.image ? this.props.pageData.imageSuffix + comment.image : null), edit_comment_id: isReply, edit_reply_comment_id: 0 })
            }
        }
    }
    postEditComment = () => {
    ...
    render() {
        ...
        let moderation = null
        if (this.props.pageData && this.props.pageData.loggedInUserDetails) {
            if(this.state.owner_id == this.props.pageData.loggedInUserDetails.user_id && this.props.pageData.appSettings['enable_comment_approve'] == 1){
        ...
        return (
            <React.Fragment>
                ...
                <div className="VideoDetails-commentWrap">
                    ...
                    {
                        this.props.pageData.appSettings['above_comment'] ? 
                            <AdsIndex paddingTop="20px" className="above_comment" ads={this.props.pageData.appSettings['above_comment']} />
                        : null
                    }
                    ...
                    {
                        this.props.pageData.appSettings['below_comment'] ? 
                            <AdsIndex paddingTop="20px" className="below_comment" ads={this.props.pageData.appSettings['below_comment']} />
                        : null
                    }
