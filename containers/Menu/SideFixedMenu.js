...
class SideFixedMenu extends React.Component{
    ...
    render(){
        ...
        if(this.props.pageData.appSettings['fixed_header'] != 1){
            return null;
        }
        let bottomFooterMenus = this.props.pageData.menus.bottomFooterMenus
        let socialShareMenus = this.props.pageData.menus.socialShareMenus
        let menus = null
        if(this.props.pageData.menus && this.props.pageData.menus.menus){
            menus = this.props.pageData.menus.menus.map(elem => {
                return (
                    <li onClick={(e) => {this.openMenu(e,elem.submenus)}} className={"nav-item" + (elem.submenus ? " dropdown dropdownmenu" : "")+ (this.props.menuOpen && elem.submenus ? "" : "") } key={elem.menu_id}>
        ...
        var hideSmallMenu = this.props.hideSmallMenu || this.props.pageData.hideSmallMenu
        ...
        return (
            <div className={`sidebar-menu${menuOpen ? " mini-menu" : ""}`}>
                ...
                <div className="menu-list sidebar-scroll">
                    ...
                    {
                        this.props.pageData.channelSubscriptions && this.props.pageData.channelSubscriptions.length > 0 ? 
                            <ul className={`main-menu border-sidebar${menuOpen ? " hide" : ""}`}>
                                 <li className={`sidebar-menu-title`}>{Translate(this.props,'Subscriptions')}</li>
                                {
                                    this.props.pageData.channelSubscriptions.map(channel => {
                                        return (
                                            <li key={channel.custom_url}>
                                                 <Link href="/channel" customParam={`id=${channel.custom_url}`} as={`/channel/${channel.custom_url}`}>
                                                    <a className="sidebar-icon">
                                                        <Image className="sidebar-img" height="24" width="24" imageSuffix={this.props.pageData.imageSuffix} image={channel.image} title={channel.title} siteURL={this.props.pageData.siteURL} />
                    ...
                    {
                        this.props.pageData.popularMembers ? 
                            <ul className={`main-menu border-sidebar${menuOpen ? " hide" : ""}`}>
                                <li className="sidebar-menu-title">{Translate(this.props,'Popular Members')}</li>
                                {
                                    this.props.pageData.popularMembers.map((member,index) => {
                                        if(index < 25){
                                            return (
                                                <li key={member.user_id}>
                                                    <Link href="/member" customParam={`id=${member.username}`} as={`/${member.username}`}>
                                                        <a className="sidebar-icon">
                                                            <Image className="sidebar-img" height="24" width="24" imageSuffix={this.props.pageData.imageSuffix} image={member.avtar} title={this.props.t(member.displayname)} siteURL={this.props.pageData.siteURL} />
                    ...
                    {
                        this.props.pageData.categoriesVideo ? 
                            <ul className="main-menu border-sidebar">
                                <li className="sidebar-menu-title">{Translate(this.props,'Categories')}</li>
                                {
                                    this.props.pageData.categoriesVideo.map(category => {
                                        return (
                                            <li key={category.category_id}>
                                                <Link href={`/category`} customParam={`type=video&id=` + category.slug} as={`/video/category/` + category.slug}>
                                                    <a className="sidebar-icon">
                                                        <Image className="sidebar-img" height="24" width="24" imageSuffix={this.props.pageData.imageSuffix} image={category.image} title={this.props.t(category.title)} siteURL={this.props.pageData.siteURL} />
                    ...
                    {
                        ( (this.props.pageData.appSettings['video_adult'] == 1 || this.props.pageData.appSettings['channel_adult'] == 1 || this.props.pageData.appSettings['blog_adult'] == 1 || this.props.pageData.appSettings['playlist_adult'] == 1) || ( this.props.pageData.languages && this.props.pageData.languages.length > 1 ) )  ?
                        <ul className="main-menu border-sidebar">
                            <li className="sidebar-menu-title">{Translate(this.props,'Settings')}</li>
                            <LanguageSwitcher {...this.props} />
                            {
                                this.props.pageData.appSettings['video_adult'] == 1 || this.props.pageData.appSettings['channel_adult'] == 1 || this.props.pageData.appSettings['blog_adult'] == 1 || this.props.pageData.appSettings['playlist_adult'] == 1 ?
                        ...
                        <div className="copyright">
                            {this.props.t("Copyright © {{year}} {{site_title}}. All Rights Reserved.",{year:(new Date()).getFullYear(),site_title:this.props.pageData.appSettings['site_title']})}
                        </div>
