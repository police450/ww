...
class Videos extends React.Component {
    ...
    render() {
        ...
        return (
            <React.Fragment>
                {
                    !this.state.playlist && this.props.pageData.appSettings['video_autoplay'] == 1 && this.props.pageData.appSettings['enable_iframely'] == 0? 
                ...
                }
                {
                    this.state.videos.map(video => {
                        ...
                        if(this.props.pageData.livestreamingtype == 0 && video.mediaserver_stream_id &&  !video.orgImage && video.is_livestreaming == 1 && parseInt(this.props.pageData.appSettings['antserver_media_hlssupported']) == 1){
                            if(this.props.pageData.liveStreamingCDNServerURL){
                                videoImage = `${this.props.pageData.liveStreamingCDNServerURL}/${this.props.pageData.streamingAppName}/previews/${video.mediaserver_stream_id}.png`
                            }else{
                                videoImage = `${this.props.pageData.liveStreamingServerURL}:5443/${this.props.pageData.streamingAppName}/previews/${video.mediaserver_stream_id}.png`  
                            }
                            ...
                            if(this.props.pageData.liveStreamingCDNURL){
                                videoImage = `${this.props.pageData.liveStreamingCDNURL}${video.image.replace(`/LiveApp`,'').replace(`/WebRTCAppEE`,'')}`
                            }else
                                videoImage = `${this.props.pageData.liveStreamingServerURL}:5443${video.image}`
                        }
                        return (
                            <div key={video.video_id} className="sidevideoWrapOutr">
                                ...
                                <div className="videoList_thumb" >
                                    <Link href="/watch" customParam={`id=${video.custom_url}`} as={`/watch/${video.custom_url}`}>
                                        <a>
                                            <Image title={CensorWord("fn",this.props,video.title)} image={videoImage} imageSuffix={this.props.pageData.imageSuffix}  siteURL={this.props.pageData.siteURL} />
                                        </a>
                                    </Link>
                                    {
                                        video.duration ?
                                    ...
                                    {
                                        this.props.pageData.appSettings["videos_playlist"] == 1 && this.props.pageData.appSettings["enable_playlist"] == 1 && (!this.props.pageData.levelPermissions || this.props.pageData.levelPermissions['playlist.create'] == 1) ?
