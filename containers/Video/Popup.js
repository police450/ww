                                        ...
                                        >
                                        {
                                            this.state.content.map(result =>{
                                                ...
                                                if(result.image.indexOf("http://") == 0 || result.image.indexOf("https://") == 0){
                                                    ...
                                                    if(this.props.pageData.livestreamingtype == 0 && result.mediaserver_stream_id &&  !result.orgImage && result.is_livestreaming == 1 && parseInt(this.props.pageData.appSettings['antserver_media_hlssupported']) == 1){
                                                        if(this.props.pageData.liveStreamingCDNServerURL){
                                                            videoImage = `${this.props.pageData.liveStreamingCDNServerURL}/${this.props.pageData.streamingAppName}/previews/${result.mediaserver_stream_id}.png`
                                                        }else
                                                            videoImage = `${this.props.pageData.liveStreamingServerURL}:5443/${this.props.pageData.streamingAppName}/previews/${result.mediaserver_stream_id}.png`
                                                    }else if(this.props.pageData.livestreamingtype == 0 && result.mediaserver_stream_id &&  result.image && result.image.indexOf(`${this.props.pageData.streamingAppName}/previews`) > 0){
                                                        if(this.props.pageData.liveStreamingCDNURL){
                                                            imageUrl = this.props.pageData.liveStreamingCDNURL+result.image.replace(`/LiveApp`,'').replace(`/WebRTCAppEE`,'')
                                                        }else
                                                            imageUrl = this.props.pageData.liveStreamingServerURL+":5443"+result.image
                                                    }else{
                                                        imageUrl = this.props.pageData.imageSuffix+result.image
                                                    }
                                                }
                                                return (
                                    ...
                                    >
                                    {
                                        this.state.myContent.map(result =>{
                                            ...
                                            if(result.image.indexOf("http://") == 0 || result.image.indexOf("https://") == 0){
                                                ...
                                                if(this.props.pageData.livestreamingtype == 0 && result.mediaserver_stream_id &&  result.image && result.image.indexOf(`${this.props.pageData.streamingAppName}/previews`) > 0){
                                                    if(this.props.pageData.liveStreamingCDNURL){
                                                        imageUrl = this.props.pageData.liveStreamingCDNURL+result.image.replace(`/LiveApp`,'').replace(`/WebRTCAppEE`,'')
                                                    }else
                                                        imageUrl = this.props.pageData.liveStreamingServerURL+":5443"+result.image
                            ...
                            </div>
                            {
                                ...
                                this.state.playlist ? 
                                    <div className={`upldPopupVdo${this.state.selectedVideo.indexOf(this.state.content[0].playlist_id ? this.state.content[0].playlist_id : this.state.content[0].video_id) > -1 ? " popup_content_selected" : ""}`} key={this.state.content[0].playlist_id ? this.state.content[0].playlist_id : this.state.content[0].video_id} onClick={this.selectedVideo.bind(this,this.state.content[0].playlist_id ? this.state.content[0].playlist_id : this.state.content[0].video_id)}>
                                        <img style={{height:"100px",width:"100px"}} src={ /^((http|https):\/\/)/.test(this.state.content[0].image) ? this.state.content[0].image : this.props.pageData.imageSuffix+this.state.content[0].image} />
                                        <div>
