    ...
    findAll: function(req,data){
        return new Promise(function(resolve, reject) {
            req.getConnection(function(err,connection){
                ...
                sql += "UNION ALL "
                sql += 'SELECT transactions.transaction_id as id,userdetails.username,userdetails.displayname,"user" as type,transactions.price as amount,admin_commission,transactions.creation_date,u.username as custom_url,u.displayname as title,transactions.type as transType FROM `transactions` INNER JOIN userdetails on userdetails.user_id = transactions.owner_id INNER JOIN userdetails u ON u.user_id = transactions.id WHERE (transactions.type = "user_subscribe") && (state = "approved" || state = "completed") AND transactions.id = ? AND transactions.price > 0  ' 
                sql += "UNION ALL "
                condition.push(parseInt(data.user_id))
                sql += 'SELECT transactions.transaction_id as id,userdetails.username,userdetails.displayname,transactions.type,transactions.price as amount,admin_commission,transactions.creation_date,movies.title,movies.custom_url,transactions.type as transType FROM `transactions` INNER JOIN userdetails on userdetails.user_id = transactions.owner_id INNER JOIN movies ON movies.movie_id = transactions.id WHERE ( transactions.type = "purchase_movie_purchase" || transactions.type = "rent_movie_purchase" || transactions.type = "purchase_series_purchase" || transactions.type = "rent_series_purchase" )   && (state = "approved" || state = "completed") AND movies.owner_id = ?  ' 
