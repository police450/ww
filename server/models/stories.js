...
module.exports = {
    ...
    getUserStories: function(req,data = {}){
        return new Promise(function(resolve, reject) {
            req.getConnection(function(err,connection){
                let select = "SELECT *,userdetails.displayname as user_displayname,userdetails.username as user_username,IF(userdetails.avtar IS NULL || userdetails.avtar = '',(SELECT value FROM `level_permissions` WHERE name = CASE WHEN userdetails.gender = 'male' THEN 'default_mainphoto' WHEN userdetails.gender = 'female'  THEN 'default_femalemainphoto' END  AND type = \"member\" AND level_id = users.level_id),userdetails.avtar) as avtar,MAX(story_id) as max_story_id,GROUP_CONCAT(story_id) as ids FROM stories INNER JOIN users ON users.user_id = stories.owner_id INNER JOIN userdetails ON userdetails.user_id = users.user_id WHERE 1=1 AND  users.approve = 1 AND users.active = 1"
                let conditions = []
                let days = req.appSettings['stories_duration'] ? req.appSettings['stories_duration'] : 1
                let currentDate = dateTime.create().format("Y-m-d H:M:S")
