...
exports.cronFunction = async(req,res,next) => {
    return new Promise(function(resolve, reject) { 
        req.getConnection(function(err,connection){
            ...
            connection.query("UPDATE tasks SET started = 0,start_time = ? WHERE (type = ? || type = ? || type = ? || type = ? || type = ? || type = ? || type = ?) AND started = 1 AND DATE_ADD(start_time, INTERVAL 6 hour) <= ? ", [dateTime.create().format("Y-m-d H:M:S"),"videoEncode","movieVideoEncode","movieImportIMDB","movieImportChangeIMDB","autoDeleteVideos","autoDeleteImportedVideos","newsletters",dateTime.create().format("Y-m-d H:M:S")], async function (err, results, fields) {
