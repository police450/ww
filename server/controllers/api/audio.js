    ...
    var command =
        ffmpeg.ffprobe(filePath, function (err, metadata) {   
            ...
            globalModel.create(req, audioObject, "audio").then(result => {
                res.send({audio_url:audio_url,  id: result.insertId,  name: path.basename(metadata.format.filename, path.extname(metadata.format.filename)) })
            })
        });
...
exports.peekData = async(req,res) => {
    ...
    if(req.body.peaks){
        ...
        await globalModel.update(req, insertObject, "audio", 'audio_id', id).then(async () => {
    ...
    if (id) {
        await globalModel.update(req, insertObject, "audio", 'audio_id', id).then(async () => {
            if(Object.keys(audioObject).length && !audioObject.image){
                ...
                notificationModel.insertFollowNotifications(req,{subject_type:"users",subject_id:req.user.user_id,object_type:"audio",object_id:id,type:"members_followed"}).then(() => {

                }).catch(() => {

                })
                return res.send({ id: id, message: constant.audio.SUCCESS, custom_url: insertObject["custom_url"] ? insertObject["custom_url"] : audioObject.custom_url });
            }


            res.send({ id: id, message: constant.audio.EDIT, custom_url: insertObject["custom_url"] ? insertObject["custom_url"] : audioObject.custom_url });
        }).catch((err) => {
        ...
        await globalModel.create(req, insertObject, "audio").then(async result => {
            if (result) {
                ...
                res.send({ id: result.insertId, message: constant.audio.SUCCESS, custom_url: insertObject['custom_url'] });
            } else {
                return res.send({ error: fieldErrors.errors([{ msg: constant.general.DATABSE }], true), status: errorCodes.invalid }).end();
            }
        }).catch(() => {
