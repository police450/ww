...
router.post('/messages/delete',isLogin,multer().none(),controller.deleteChat);
...
router.post('/message/create',isLogin,(req,res,next) => {
    // if(req.body.video){   
        ...
        currUpload(req, res, function (err) {
            if (err) {
                ...
                if (req.file && req.appSettings.upload_system != "s3" && req.appSettings.upload_system != "wisabi") {
                    ...
                    const pathName = req.serverDirectoryPath + "/public/upload/images/video/"
