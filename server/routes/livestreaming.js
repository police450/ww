...
router.get(process.env.subFolder+':lng?/live-streaming/:custom_url?',multer().none(),async (req,res,next) => {
    middlewareEnable.isEnable(req,res,next,"livestreaming",'create')
},controller.create);


router.get(process.env.subFolder+':lng?/live',multer().none(),(req,res,next) => {
